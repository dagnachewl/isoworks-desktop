"""
ams_physical_prep_create_run_gui.py — Physical Prep batch creation dialog.

Sources available samples from public.sample_queue (workflowjob.jobname = 'Physical Prep').
Assembles a positional batch layout and saves to ams.physicalprep_run + ams.physicalprep_sample,
consuming the corresponding sample_queue rows.

Layout mirrors ams_graphitisation_create_run_gui: left=setup+available, centre=buttons,
right=batch layout. Unlike Graphitisation, there is no catalyst-equivalent field chosen
at creation time -- prep_method/masses are entered later in Run Details.
"""
...
# ── Batch-layout table column indices (no catalyst column, unlike Graphitisation) ──
...
class PhysicalPrepCreateRunDialog(QDialog):
    """
    Assemble a new physical prep batch from the TBA queue.

    Left   — Run Setup (metadata) + Available Samples (sample_queue, Physical Prep job)
    Centre — transfer buttons (>> / >>> / < / <<)
    Right  — Batch layout (positions with sample assignments)

    Save inserts ams.physicalprep_run + ams.physicalprep_sample and deletes
    consumed sample_queue rows.
    """

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Create Physical Prep Batch")
        self.setModal(True)
    ...
    def _init_ui(self):
        ...
        # top bar
        bar = QHBoxLayout()
        bar.addWidget(QLabel("<b>Create Physical Prep Batch</b>"))
        bar.addStretch()
        ...
        for w in [self.btnSave, btnCancel,
                  make_help_button(self, "physicalprep_create_run")]:
            bar.addWidget(w)
        root.addLayout(bar)
    ...
    def _build_run_setup(self) -> QGroupBox:
        ...
        self.txtBatchCode.setPlaceholderText("e.g. PP260115A")
    ...
    def _build_available_group(self) -> QGroupBox:
        grp = QGroupBox("Available Samples  (TBA queue — Physical Prep job)")
    ...
    def _load_lookups(self):
        try:
            with db_manager.get_connection() as conn:
                ...
                # categoryid 15 = Physical Prep (job_procedure renumber,
                # migration 080) -- scopes this picker to Physical Prep
                ...
                for r in conn.execute(text(
                    ...
                    "WHERE jp.id = 15 AND e.IsObsolete IS NOT TRUE "
                ...
                # Workflow filter — only AMS workflows that have a Physical Prep job
                for r in conn.execute(text("""
                    ...
                    WHERE wj.jobname = 'Physical Prep' AND w.isobsolete = FALSE
                    ORDER BY w.workflowname
                """)):
                    self.cmbWfFilter.addItem(r[1], r[0])

        except Exception as exc:
            log.error("PhysicalPrepCreateRunDialog._load_lookups: %s", exc)
    ...
    def _suggest_batch_code(self):
        ...
        sug = f"PP{dt.toString('yyMMdd')}A"
        if not cur or (cur.startswith("PP") and len(cur) == 9):
    ...
    def _load_available_samples(self):
        ...
        params: dict = {}
        clauses = ["wj.jobname = 'Physical Prep'"]
    ...
    def _save(self):
        ...
        try:
            with db_manager.get_connection() as conn:
                # 1. ams.physicalprep_run
                run_row = conn.execute(text("""
                    INSERT INTO ams.physicalprep_run
                ...
                # 2. ams.physicalprep_sample — one per filled position
                for info in filled:
            ...
            log.info("Created physical prep batch %d (%s) with %d samples",
            ...
            log.error("PhysicalPrepCreateRunDialog._save: %s", exc)
            if "uq_physicalprep_run_batchcode" in str(exc).lower():
