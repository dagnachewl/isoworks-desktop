"""
ams_pretreatment_create_run_gui.py — Pretreatment batch creation dialog.

Sources available samples from public.sample_queue (workflowjob.jobname ILIKE
'%pretreat%', matching the web backend's own filter for this stage).
Assembles a positional batch layout and saves to ams.pretreatmentrun +
ams.pretreatmentsample, consuming the corresponding sample_queue rows.

Unlike Physical Prep/Purification, Pretreatment batches are single-method:
the Method is chosen once for the whole batch here (not per-row), matching
ams.pretreatmentrun.method (NOT NULL) and stamped onto every sample row.

Layout mirrors ams_physical_prep_create_run_gui: left=setup+available,
centre=buttons, right=batch layout.
"""
...
class PretreatmentCreateRunDialog(QDialog):
    """
    Assemble a new pretreatment batch from the TBA queue.

    Left   — Run Setup (metadata + batch-level Method) + Available Samples
    Centre — transfer buttons (>> / >>> / < / <<)
    Right  — Batch layout (positions with sample assignments)

    Save inserts ams.pretreatmentrun + ams.pretreatmentsample (stamping the
    chosen method onto every row) and deletes consumed sample_queue rows.
    """

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Create Pretreatment Batch")
        self.setModal(True)
    ...
    def _init_ui(self):
        ...
        # top bar
        bar = QHBoxLayout()
        bar.addWidget(QLabel("<b>Create Pretreatment Batch</b>"))
        bar.addStretch()
        ...
        for w in [self.btnSave, btnCancel,
                  make_help_button(self, "pretreatment_create_run")]:
            bar.addWidget(w)
        root.addLayout(bar)
    ...
    def _build_run_setup(self) -> QGroupBox:
        ...
        self.txtBatchCode.setPlaceholderText("e.g. PT260115A")
        ...
        # Batch size + method on the same row -- the whole batch shares one
        # pretreatment method (ams.pretreatmentrun.method, NOT NULL).
        ...
        size_row.addWidget(QLabel("Method:"))
        self.cmbMethod = QComboBox()
        self.cmbMethod.addItems(_METHODS)
        size_row.addWidget(self.cmbMethod)
        size_row.addStretch()
        form.addRow("Batch Size / Method", size_row)
    ...
    def _build_available_group(self) -> QGroupBox:
        grp = QGroupBox("Available Samples  (TBA queue — Pretreatment job)")
    ...
    def _load_lookups(self):
        try:
            with db_manager.get_connection() as conn:
                ...
                # categoryid 16 = Pretreatment (job_procedure renumber,
                # migration 080) -- scopes this picker to Pretreatment
                ...
                for r in conn.execute(text(
                    ...
                    "WHERE jp.id = 16 AND e.IsObsolete IS NOT TRUE "
                ...
                # Workflow filter — only AMS workflows that have a Pretreatment job.
                # ILIKE '%pretreat%' matches the web backend's own filter for this
                # stage (backend/app/api/ams.py get_available_for_pretreatment).
                for r in conn.execute(text("""
                    ...
                    WHERE wj.jobname ILIKE '%pretreat%' AND w.isobsolete = FALSE
                    ORDER BY w.workflowname
                """)):
                    self.cmbWfFilter.addItem(r[1], r[0])

        except Exception as exc:
            log.error("PretreatmentCreateRunDialog._load_lookups: %s", exc)
    ...
    def _suggest_batch_code(self):
        ...
        sug = f"PT{dt.toString('yyMMdd')}A"
        if not cur or (cur.startswith("PT") and len(cur) == 9):
    ...
    def _load_available_samples(self):
        ...
        params: dict = {}
        clauses = ["wj.jobname ILIKE '%pretreat%'"]
    ...
    def _save(self):
        ...
        try:
            with db_manager.get_connection() as conn:
                # 1. ams.pretreatmentrun
                run_row = conn.execute(text("""
                    INSERT INTO ams.pretreatmentrun
                        (batchcode, batchdate, equipmentid, technicianid,
                         notes, runstatus, islocked, method,
                         createdatestamp, createuserstamp)
                    VALUES
                        (:code, :dt, :eid, :tid,
                         :notes, 0, FALSE, :method,
                         :now, :user)
                    RETURNING pretreatmentrunid
                """), {
                    "code": code, "dt": batch_dt,
                    "eid": equip_id, "tid": tech_id,
                    "notes": notes_txt, "method": method,
                    "now": now, "user": user,
                }).fetchone()
                batch_id = run_row[0]
                ...
                # 2. ams.pretreatmentsample — one per filled position
                for info in filled:
            ...
            log.info("Created pretreatment batch %d (%s, method=%s) with %d samples",
                     batch_id, code, method, len(filled))
            show_message(self, "Batch Created",
                         f"Batch {batch_id} ({code}) created with {len(filled)} sample(s).")
            self.accept()

        except Exception as exc:
            log.error("PretreatmentCreateRunDialog._save: %s", exc)
            if "uq_pretreatmentrun_batchcode" in str(exc).lower():
