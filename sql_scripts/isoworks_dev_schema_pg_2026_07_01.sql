...
\restrict fH2ipfnke0Uvw9EjqyRDXQmJwHs6TWOaTjzZgZOgkx7yfFEyJ5CFVV07jcgoLPt
...
--
-- Name: fn_sync_submission_status(); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.fn_sync_submission_status() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
...
BEGIN
    ...
    IF    v_min_status >= 220 THEN v_new_status := 220;
    ELSIF v_min_status >= 210 THEN v_new_status := 210;
    ELSE  RETURN NEW;   -- not yet fully done — leave submission as-is
    END IF;
...
ALTER FUNCTION public.fn_sync_submission_status() OWNER TO postgres;
...
CREATE TABLE public.finalvalue (
    ...
    analyteid smallint DEFAULT 0 CONSTRAINT finalvalue_measurableid_not_null NOT NULL,
    measurableunit smallint,
    fvalue double precision,
    fvalueunc double precision,
    lldstatus smallint,
...
ALTER TABLE ONLY public.finalvalue
    ADD CONSTRAINT finalvalue_pkey PRIMARY KEY (analysisid, analyteid);
...
--
-- Name: sample trg_sample_status_sync_submission; Type: TRIGGER; Schema: public; Owner: postgres
--

CREATE TRIGGER trg_sample_status_sync_submission AFTER UPDATE OF status ON public.sample FOR EACH ROW WHEN (((old.status IS DISTINCT FROM new.status) AND (new.sampletype = 0))) EXECUTE FUNCTION public.fn_sync_submission_status();
...
\unrestrict fH2ipfnke0Uvw9EjqyRDXQmJwHs6TWOaTjzZgZOgkx7yfFEyJ5CFVV07jcgoLPt
