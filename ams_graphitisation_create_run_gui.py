"""
ams_graphitisation_create_run_gui.py — Graphitisation batch creation dialog.

Sources available samples from public.sample_queue (workflowjob.jobname = 'Graphitization').
Assembles a positional batch layout and saves to ams.graphrun + ams.graphsample,
consuming the corresponding sample_queue rows.

Layout mirrors ams_create_run_gui: left=setup+available, centre=buttons, right=batch layout.
"""
...
LL_CAT  = 4   # catalyst type combo widget
LL_BTN  = 5   # clear button widget


class GraphitisationCreateRunDialog(QDialog):
    """
    Assemble a new graphitisation batch from the TBA queue.

    Left   — Run Setup (metadata) + Available Samples (sample_queue, Graphitization job)
    Centre — transfer buttons (>> / >>> / < / <<)
    Right  — Batch layout (positions with sample assignments and catalyst type)

    Save inserts ams.graphrun + ams.graphsample and deletes consumed sample_queue rows.
    """

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Create Graphitisation Batch")
        self.setModal(True)
    ...
    def _init_ui(self):
        ...
        # top bar
        bar = QHBoxLayout()
        bar.addWidget(QLabel("<b>Create Graphitisation Batch</b>"))
        bar.addStretch()
        ...
        for w in [self.btnSave, btnCancel,
                  make_help_button(self, "graphitisation_create_run")]:
            bar.addWidget(w)
        root.addLayout(bar)
    ...
    def _build_run_setup(self) -> QGroupBox:
        ...
        self.txtBatchCode.setPlaceholderText("e.g. GR260115A")
        ...
        # Batch size + default catalyst on the same row
        ...
        size_row.addWidget(self.spnSize)
        size_row.addSpacing(12)
        size_row.addWidget(QLabel("Default Catalyst:"))
        self.cmbDefaultCat = QComboBox()
        self.cmbDefaultCat.addItems(_CATALYSTS)
        size_row.addWidget(self.cmbDefaultCat)
        size_row.addStretch()
        form.addRow("Batch Size / Catalyst", size_row)
    ...
    def _build_available_group(self) -> QGroupBox:
        grp = QGroupBox("Available Samples  (TBA queue — Graphitization job)")
    ...
    def _build_batch_group(self) -> QGroupBox:
        ...
        self.modelBatch.setHorizontalHeaderLabels(
            ["Pos", "AnalysisID", "Sample Ref", "Sample Name", "Catalyst", ""]
        )
    ...
    def _apply_batch_col_widths(self):
        ...
        h.setSectionResizeMode(LL_CAT,  QHeaderView.Fixed);  self.tblBatch.setColumnWidth(LL_CAT,  70)
    ...
    def _load_lookups(self):
        try:
            with db_manager.get_connection() as conn:
                ...
                # Workflow filter — only AMS workflows that have a Graphitization job
                for r in conn.execute(text("""
                    ...
                    WHERE wj.jobname = 'Graphitization' AND w.isobsolete = FALSE
                    ORDER BY w.workflowname
                """)):
                    self.cmbWfFilter.addItem(r[1], r[0])

        except Exception as exc:
            log.error("GraphitisationCreateRunDialog._load_lookups: %s", exc)
    ...
    def _suggest_batch_code(self):
        ...
        sug = f"GR{dt.toString('yyMMdd')}A"
        if not cur or (cur.startswith("GR") and len(cur) == 9):
            self.txtBatchCode.setText(sug)

    def _load_available_samples(self):
        self.modelAvail.removeRows(0, self.modelAvail.rowCount())

        # Exclude analysisids already assigned in the batch layout
        ...
        params: dict = {}
        clauses = ["wj.jobname = 'Graphitization'"]
    ...
    def _build_positions(self):
        """Rebuild the batch layout table with _n_positions empty rows."""
        # Collect already-assigned samples so we don't lose them on resize
        preserved: dict[int, dict] = {}   # pos (1-based) → assignment info
        for info in self._batch_rows:
            if info.get("aid") is not None:
        ...
        default_cat = self.cmbDefaultCat.currentText()
        ...
        for pos in range(1, self._n_positions + 1):
            ...
            cat_item  = QStandardItem("")   # widget placeholder
            btn_item  = QStandardItem("")

            for it in [pos_item, aid_item, sref_item, name_item, cat_item, btn_item]:
                it.setBackground(QBrush(bg))
                it.setFlags(it.flags() & ~Qt.ItemIsEditable)

            aid_item.setData(aid,   Qt.UserRole)      # analysisid
            aid_item.setData(tbaid, Qt.UserRole + 1)  # queue_id

            row_idx = self.modelBatch.rowCount()
            self.modelBatch.appendRow(
                [pos_item, aid_item, sref_item, name_item, cat_item, btn_item]
            )

            # Catalyst combo widget
            cmb = QComboBox()
            cmb.addItems(_CATALYSTS)
            cmb.setCurrentText(default_cat)
            cmb.setEnabled(True)
            self.tblBatch.setIndexWidget(
                self.modelBatch.index(row_idx, LL_CAT), cmb
            )
    ...
    def _save(self):
        ...
        # Collect catalyst per row (from QComboBox widgets)
        catalysts: dict[int, str] = {}   # batch_row → catalyst
        for row_idx in range(self.modelBatch.rowCount()):
            cmb = self.tblBatch.indexWidget(self.modelBatch.index(row_idx, LL_CAT))
            catalysts[row_idx] = cmb.currentText() if cmb else "Fe"
        ...
        try:
            with db_manager.get_connection() as conn:
                ...
                # 2. ams.graphsample — one per filled position
                for info in filled:
                    row_idx = info["row"]
                    cat     = catalysts.get(row_idx, "Fe")
            ...
            log.info("Created graphitisation batch %d (%s) with %d samples",
            ...
            log.error("GraphitisationCreateRunDialog._save: %s", exc)
