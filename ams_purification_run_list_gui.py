"""
ams_purification_run_list_gui.py — Purification batch list for IsoWorks AMS.
Provides PurificationRunListWindow (QWidget) for browsing, creating and
opening purification batches.
"""
...
try:
    from ams_purification_run_details_gui import PurificationRunDetailsWindow
except ImportError:
    PurificationRunDetailsWindow = None

try:
    from ams_purification_create_run_gui import PurificationCreateRunDialog
except ImportError:
    PurificationCreateRunDialog = None
...
class PurificationRunListWindow(QWidget):
    """Browse, create and open purification batches."""
    ...
    def _check_privileges(self):
        try:
            ...
            log.error("Purification privilege check: %s", exc)
    ...
    def _build_action_bar(self) -> QHBoxLayout:
        ...
        for w in [self.btnCreate, self.btnDelete, btnClose,
                  make_help_button(self, "purification_run_list")]:
            bar.addWidget(w)
        return bar

    def _build_filter_group(self) -> QGroupBox:
        grp = QGroupBox("Filter / Search Purification Batches")
        ...
        try:
            with db_manager.get_connection() as conn:
                rows = conn.execute(text("""
                    ...
                    FROM Equipment e
                    INNER JOIN ams.purification_run r ON r.equipmentid = e.EquipmentID
            ...
            log.warning("Purification filter combos: %s", exc)
            self.cmbEquipFilter.addItem("— All Equipment —", None)
    ...
    def load_run_list(self):
        ...
        if sval:
            try:
                if stype == "Batch ID":
                    clauses.append("r.purificationrunid = :bid")
                    params["bid"] = int(sval)
                    ...
                    clauses.append("""
                        r.purificationrunid IN (
                            SELECT ps.purificationrunid FROM ams.purification_sample ps
        ...
        sql = f"""
            SELECT r.purificationrunid, r.batchcode, r.batchdate,
                   e.EquipmentName,
                   {db_manager.sql_concat('emp.LastName', "', '", 'emp.FirstMiddleName')} AS technician,
                   r.runstatus, r.notes,
                   COUNT(ps.purificationsampleid)                           AS nsamples,
                   COUNT(ps.purificationsampleid) FILTER (WHERE ps.isaccepted) AS naccepted
            FROM ams.purification_run r
            LEFT JOIN Equipment  e   ON e.EquipmentID  = r.equipmentid
            LEFT JOIN Employee   emp ON emp.EmployeeID = r.technicianid
            LEFT JOIN ams.purification_sample ps ON ps.purificationrunid = r.purificationrunid
            {where}
            GROUP BY r.purificationrunid, r.batchcode, r.batchdate,
                     e.EquipmentName, emp.LastName, emp.FirstMiddleName,
                     r.runstatus, r.notes
            ORDER BY r.purificationrunid DESC
        """
        ...
        try:
            ...
            log.error("Purification load_run_list: %s", exc)
            return
    ...
    def _create_batch(self):
        if PurificationCreateRunDialog is None:
            show_message(self, "Missing", "ams_purification_create_run_gui not found.")
            return
        dlg = PurificationCreateRunDialog(self)
    ...
    def _open_batch_details(self, batch_id: int):
        if PurificationRunDetailsWindow is None:
            show_message(self, "Missing",
                         "ams_purification_run_details_gui not found.")
            return
        ...
        win = PurificationRunDetailsWindow(batch_id=batch_id, parent=self)
    ...
    def _delete_batch(self):
        ...
        if QMessageBox.question(
            self, "Confirm Delete",
            f"Permanently delete Purification Batch {bid} and all its samples?\n"
        ...
        try:
            with db_manager.get_connection() as conn:
                conn.execute(text(
                    "DELETE FROM ams.purification_sample WHERE purificationrunid = :bid"
                ), {"bid": bid})
                conn.execute(text(
                    "DELETE FROM ams.purification_run WHERE purificationrunid = :bid"
                ), {"bid": bid})
                conn.commit()
            self.load_run_list()
        except Exception as exc:
            log.error("Delete purification_run %d: %s", bid, exc)
