"""
ngam_ingrowth_runs_gui.py
=========================
NGAM – Noble Gas Analysis Module
3H-3He Ingrowth Run List

Mirrors trims_electrolysis_runs_gui.py pattern exactly.
...
try:
    from ngam_ingrowth_run_gui import NGAMIngrowthRunWindow
except ImportError:
    NGAMIngrowthRunWindow = None
...
# ─────────────────────────────── main window ─────────────────────────────────
class NGAMIngrowthRunsWindow(QWidget):
    """
    List, filter and open 3H-3He Ingrowth Runs.
    ...
    def __init__(self, parent=None):
        ...
        top_bar.addWidget(make_help_button(self, "ngam_ingrowth"))
    ...
    def _check_privileges(self):
        try:
            ...
            logging.info(f"NGAM user {user}: write={self.has_write_priv}, admin={self.has_admin_priv}")
        except Exception as e:
            logging.error(f"NGAM privilege check failed: {e}", exc_info=True)
            self.has_write_priv = False
            self.has_admin_priv = False
    ...
    def _create_filter_group(self):
        grp = QGroupBox("Filter / Search 3He Ingrowth Runs")
        layout = QVBoxLayout()
        ...
        self.cmbSearchType.addItems(["Ingrowth Run", "Sample ID", "Sample Name", "Project Name"])
        ...
        self.btnSearch = QPushButton("Open Run")   # smart: changes label on combo switch
    ...
    def _on_search_type_changed(self, text: str):
        self.btnSearch.setText("Open Run" if text == "Ingrowth Run" else "Search")

    def _on_search_or_open(self):
        if self.cmbSearchType.currentText() == "Ingrowth Run":
            ...
            try:
                ...
                show_message(self, "Invalid ID", "Ingrowth Run ID must be a number.", QMessageBox.Warning)
                return
    ...
    def _build_query(self):
        ...
        if s_val:
            if s_type == "Ingrowth Run" and s_val.isdigit():
                wheres.append("r.RunID = :rid")
                params["rid"] = int(s_val)
            elif s_type == "Sample ID" and s_val.isdigit():
                wheres.append("""
                    r.RunID IN (
                        SELECT DISTINCT d.RunID FROM NGAM.NG3HeIngrowthData d
    ...
    def load_run_list(self):
        ...
        try:
            ...
            logging.error(f"NGAM ingrowth load_run_list failed: {e}")
    ...
    def _on_row_clicked(self, index):
        ...
        if item and item.data(Qt.UserRole) is not None:
            self.cmbSearchType.setCurrentText("Ingrowth Run")
    ...
    def create_new_run(self):
        if not NGAMIngrowthCreateRunDialog:
            ...
            return
        dlg = NGAMIngrowthCreateRunDialog(self)
        if dlg.exec_() == QDialog.Accepted:
            self.load_run_list()

    def open_run_details(self, run_id):
        if not NGAMIngrowthRunWindow:
            show_message(self, "Error", "Run Details module not loaded.", QMessageBox.Critical)
            return
        dlg = NGAMIngrowthRunWindow(run_id=int(run_id), parent=self)
    ...
    def delete_run(self):
        ...
        reply = QMessageBox.question(
            self, "Delete Run",
            f"Delete Ingrowth Run {rid} and all its sample data?",
        ...
        try:
            with db_manager.get_connection() as conn:
                ...
                sample_rows = conn.execute(text("""
                    SELECT DISTINCT analysisid FROM ngam.ng3heingrowthdata
                    WHERE runid = :rid AND analysisid IS NOT NULL
                """), {"rid": rid}).fetchall()
                analysis_ids = [r[0] for r in sample_rows]

                if analysis_ids:
                    # Guard: refuse if any ingrowth analysisid is consumed by a 3He sequence run.
                    in_sequence = conn.execute(text("""
                        SELECT DISTINCT r.runid FROM ngam.ng3hesequenceloadlist ll
                        JOIN ngam.ng3hesequencerun r ON r.runid = ll.runid
                        WHERE ll.analysisid = ANY(:aids)
                        LIMIT 1
                    """), {"aids": analysis_ids}).fetchone()
                    if in_sequence:
                        show_message(self, "Delete Blocked",
                            f"Cannot delete: ingrowth results are used by 3He sequence run #{in_sequence[0]}.\n"
                            f"Delete that sequence run first.", QMessageBox.Warning)
                        return

                    # Delete children + analysis cascade
                    conn.execute(text("DELETE FROM ngam.ng3heingrowthdata WHERE runid = :rid"), {"rid": rid})
                ...
                conn.execute(
                    text("DELETE FROM NGAM.NG3HeIngrowthRun WHERE RunID = :rid"),
                    {"rid": rid}
                )
                conn.commit()
            self.load_run_list()
        except Exception as e:
            logging.error(f"NGAM ingrowth delete_run failed: {e}")
